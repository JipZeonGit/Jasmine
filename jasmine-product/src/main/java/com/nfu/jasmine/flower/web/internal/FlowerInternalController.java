...
public class FlowerInternalController {
    ...
    @Operation(summary = "批量查询花卉基本信息（内部）")
    @PostMapping("/batch")
    public List<FlowerDTO> getFlowersByIds(@RequestBody List<Integer> ids) {
        return flowerService.listByIds(ids).stream()
                .filter(f -> f.getDeleted() == null || f.getDeleted() == 0)
                .map(this::toFlowerDTO)
                .toList();
    }

    @Operation(summary = "按名称模糊查询花卉ID列表（内部）")
    @GetMapping("/ids-by-name")
    public List<Integer> getFlowerIdsByName(@RequestParam String name) {
        if (!org.springframework.util.StringUtils.hasText(name)) {
            return List.of();
        }
        return flowerService.list(new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<Flower>()
    ...
    public ResponseEntity<StockAdjustResult> adjustStock(@RequestBody StockAdjustRequest request) {
        try {
            ...
            StockAdjustResult ok = StockAdjustResult.ok(result.beforeStock(), result.afterStock(), result.flower());
